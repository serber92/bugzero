...
class Database:
    ...
    def get_managed_products(self, managed_products_table, vendor_id, product_type_name):
        """
        ...
        entries = self.conn.query(
            ...
            self.base.classes[managed_products_table].vendorData['productTypeName'] == product_type_name
        ).all()
        return entries

    def get_managed_product_by_name(self, managed_products_table, vendor_id, product_name):
        """
    ...
    def create_managed_product(
            self, name, versions, vendor_id, managed_products_table, product_type_name, service_settings
    ):
        """
    ...
    def insert_bug_updates(self, bugs, bugs_table):
        ...
        for bug in bugs:
            ...
            if existing_bug:
                update_fields = ['description', 'status', 'knownAffectedReleases', 'knownFixedReleases']
                updated = False
                for field in update_fields:
                    if bug[field] != getattr(existing_bug, field):
                        for key, value in bug.items():
                            setattr(existing_bug, key, value)
                        existing_bug.updatedAt = datetime.datetime.utcnow()
                        counter['updated_bugs'] += 1
                        logger.info(
                            f"'{bug['vendorData']['vendorProductName']}' - updating bug | "
    ...
    def remove_non_active_managed_products(
        ...
        """
        - remove any managedProduct that no longer has an active SN CI
