...
class Test(TestCase):
    ...
    ####################################################################################################################
    @patch.dict(os.environ, mock_env())
    def test_get_service_config_missing(*_args):
        """
        when a vendor setting is missing return false
        :param _args:
        :return:
        """
        from db_client import Database
        instance = type(
            "mockDbClient",
            (object,), {
                "conn": type("mockObject", (object,),
                             {"query": lambda *args, **kwargs: type("mockObject", (object,),
                                                                    {"get": lambda *args, **kwargs: False})}),
                "base": MagicMock()
            }
        )
        instance.return_value = False
        config = Database.get_service_config(self=instance, settings_table="", vendor_id="rh")
        assert not config
        del sys.modules['db_client']
    ...
    ####################################################################################################################
    @patch.dict(os.environ, mock_env())
    def test_get_vendor_settings_false(*_args):
        """
        ...
        instance.return_value = False
        config = Database.get_vendor_settings(self=instance, vendors_table="", vendor_id="rh")
        assert not config
        del sys.modules['db_client']
    ...
    def test_get_vendor_create_managed_product(*_args):
        ...
        new_managed_product = Database.create_managed_product(
            self=instance, managed_products_table="", name="", versions="", vendor_id="", model="",
    ...
    def test_update_managed_product_versions(*_args):
        """
        testing a successful update of managedProduct
        :param _args:
        :return:
        """
        from db_client import Database
        ...
        assert not Database.update_managed_product_versions(
            self=instance, managed_product=mock_managed_product, versions=["1.1", "2.2"]
        )
        del sys.modules['db_client']

    ####################################################################################################################
    #                                           update_account_customers                                               #
    ####################################################################################################################
    @patch.dict(os.environ, mock_env())
    @patch("sqlalchemy.orm.attributes")
    def test_update_account_customers(*_args):
        """
        testing a successful update of netapp account customers that should complete without exceptions
        :param _args:
        :return:
        """
        from db_client import Database
        ...
        service_config = type(
            ...
            {"value": {"vendorPriorities": "", "vendorStatuses": "", "netAppPartnerCustomers": []}}
        )
        account_customers = {"customers": {"list": [{"customer_id": "1", "customer_name": "test"}]}}
        assert not Database.update_account_customers(
            self=instance, service_config=service_config, account_customers=account_customers
        )
        del sys.modules['db_client']
    ...
    def test_insert_bug_updates(*_args):
        ...
        assert execution["inserted_bugs"] == 1
        del sys.modules['db_client']
