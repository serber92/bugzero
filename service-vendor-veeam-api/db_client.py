...
class Database:
    ...
    def create_managed_product(
            self, name, vendor_product_id, versions, vendor_id, managed_products_table, service_settings, major_versions
    ):
        """
        ...
        if not self.conn.query(
            ...
            new_managed_product = {
                "vendorId": vendor_id,
                "vendorData": {
                    "osVersions": sorted(versions),
                    "osMajorVersions": sorted(major_versions),
    ...
    def insert_bug_updates(self, bugs, bugs_table, product_name):
        ...
        for bug in bugs:
            ...
            if existing_bug:
                if bug["description"] != existing_bug.description:
                    for key, value in bug.items():
