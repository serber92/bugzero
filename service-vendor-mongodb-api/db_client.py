...
class Database:
    ...
    def get_vendor_settings(self, vendors_table, vendor_id):
        ...
        :return:
        """
        rh_vendor_settings = self.conn.query(
            self.base.classes[vendors_table]
        ).filter_by(id=vendor_id).first()
        if not rh_vendor_settings or not rh_vendor_settings.active:
            return False
        return rh_vendor_settings

    def get_managed_products_by_vendor(self, managed_products_table, vendor_id):
        """
        get managed product by vendorId
    ...
    def update_managed_product_versions(self, managed_product, versions):
        ...
        """
        logger.info(
            f"'{managed_product.name}' - updating managed product | versions {sorted(list(versions))}"
        )
    ...
    def create_managed_product(
            self, name, versions, vendor_id, managed_products_table, service_settings, project="SERVER"
    ):
        """
        ...
        if not self.conn.query(
            ...
            new_managed_product = {
                "vendorId": vendor_id,
                "vendorData": {
                    "vendorResolutions": service_settings.value.get("vendorResolutions") or [],
                    "vendorProjects": service_settings.value.get("vendorProjects") or [project],
                    "productSoftwareVersions": versions
                },
                "vendorPriorities": service_settings.value['vendorPriorities'],
                "vendorStatuses": service_settings.value['vendorStatuses'],
                "createdAt": now_utc,
                "updatedAt": now_utc,
                "isDisabled": 0,
                "lastExecution": None,
        ...
        for bug in bugs:
            ...
            if existing_bug:
                if bug["vendorLastUpdatedDate"] != existing_bug.vendorLastUpdatedDate:
                    ...
                    logger.info(f"'{product_name}' - updating bug | {json.dumps(bug, default=str)}")
                else:
                    counter['skipped_bugs'] += 1
                    continue
            else:
                now_utc = datetime.datetime.utcnow()
                logger.info(f"'{product_name}' - inserting bug |{json.dumps(bug, default=str)}")
