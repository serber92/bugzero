...
def download_instance(
        link, session, retry=5, logger=logging.getLogger(__name__), headers=None, method='GET', payload=None, json=None
):
    """
    ...
    while tries < retry:
        try:
            if method == "POST":
                if session:
                    request = session.post(url=link, data=payload, json=json, timeout=3)
                else:
