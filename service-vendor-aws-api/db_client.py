...
class Database:
    ...
    def truncate_text_fields(table_class, key, value, default=1000):
        """
        truncate varchar or text values based on model definition or default value provided
        :param key:
        :param value:
        :param table_class:
        :param default:
    ...
    def get_managed_products(self, managed_products_table, vendor_id, product_name):
        """
    ...
    def get_vendor_settings(self, vendors_table, vendor_id):
        ...
        if not rh_vendor_settings or not rh_vendor_settings.active:
            return False
        return rh_vendor_settings.vendorData
    ...
    def create_managed_product(
        ...
        managed_product = self.conn.query(
            self.base.classes[managed_products_table]
        ).filter(self.base.classes[managed_products_table].name == name).all()
        if not managed_product:
            new_managed_product = {
                "vendorId": vendor_id,
                "vendorData": {
                    "vendorEventScopes": service_settings['vendorEventScopes'],
                },
    ...
    def insert_bug_updates(self, bugs, bugs_table, product_name):
        ...
        for bug in bugs:
            ...
            if existing_bug:
                if bug["vendorLastUpdatedDate"].date() != existing_bug.vendorLastUpdatedDate.date():
                    for key, value in bug.items():
    ...
    def update_managed_product_versions(self, managed_product, versions, major_versions):
        """
        update version attribute of a manged product
        :param managed_product:
        :param versions:
        :param major_versions:
        :return:
        """
        logger.info(
            f"'{managed_product.name}' - updating managed product | major versions/versions "
            f"{list(major_versions)}{sorted(list(versions))}"
        )
        ...
        managed_product.vendorData["osMajorVersions"] = sorted(list(major_versions))
