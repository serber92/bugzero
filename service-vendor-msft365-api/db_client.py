...
class Database:
    ...
    def update_managed_product_versions(self, managed_product, versions):
        ...
        managed_product.vendorData['productVersionIds'] = versions
        managed_product.updatedAt = datetime.datetime.utcnow()
        flag_modified(managed_product, "vendorData")
        self.conn.commit()
    ...
    def remove_non_active_managed_products(
        ...
        for product in entries:
            logger.info(
                f"'{product.name}' - removing managedProduct and associated bugs | product no longer exist in "
                f"inventory"
            )
    ...
    def get_service_entry(self, service_id, services_table):
        """
        get a service entry from the services table using by id
        :param service_id:
