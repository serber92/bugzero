...
import ServiceWidgetItem from './ServiceWidgetItem';
...
const ServiceWidget = ({ data, title }) => {
          ...
          <Col xs={5} sm={6} className="col-xxl pr-2">
            ...
            <div className="fs--2 mt-3">
              {isIterableArray(data) &&
                data.map(({ id, ...rest }) => <ServiceWidgetItem {...rest} totalShare={totalShare} key={id} />)}
            </div>
          </Col>
          <Col xs="auto">
...
ServiceWidget.propTypes = { data: PropTypes.array.isRequired };
