...
const NavbarVertical = ({ navbarStyle }) => {
  ...
  useEffect(() => {
    async function onLoad() {
      // Load Routes
      const vendors = await BZAPIService.getVendors();
        ...
        .map(vendor => {
          const vendorRoute = {
            ...
            children: [
              ...
              {
                to: `/vendors/${vendor.id}/configuration`,
                name: `Configuration`,
                exact: true
              }
            ]
      ...
      routes[routes.findIndex(el => el.name === 'Vendors')].children = vendorRoutes;
      setIsLoaded(true);
    }
    if (is.windows()) {
  ...
  return (
    ...
    >
      ...
      >
        <Nav navbar vertical>
          {isLoaded ? <NavbarVerticalMenu routes={routes} /> : null}
        </Nav>
